...
class WeiboTweetRule(object):
    ...
    def get_weibo_info(self, feed):
        ...
        str_head = feed.xpath("div")[0]
        str_node = etree.tostring(str_head)

        pattern_pic = re.compile(r'(https://weibo.cn/mblog/picAll/.*?rl=1)')
        m = pattern_pic.search(str_node)
        rep = m.group(1) if m else ''

        if rep:
            image_urls = self.get_pic_all(rep)

        elif str(str_footer.xpath("a[2]/text()")[0]).strip() == u"原图":
    ...
    def get_pic_all(self, pic_link):

        html = requests.get(pic_link, cookies=self.cookie).content
        pattern = r'<img.*\\/(.*?).jpg.*?>'
